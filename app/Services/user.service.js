...
var UserService = (function () {
    function UserService(http) {
        this.http = http;
        this.cartUpdated$ = new core_1.EventEmitter();
        var address;
        this.user = new user_model_1.User(new address_model_1.Address());
    }
    ...
    UserService.prototype.addToCart = function (productId, userId) {
        var _this = this;
        this.getById(userId)
            .subscribe(function (res) {
            _this.user = (res);
            if (!_this.user.cartItems)
                _this.user.cartItems = [];
            _this.user.cartItems.push(productId);
            _this.update(_this.user);
            _this.cartUpdated$.emit(_this.user.cartItems.length);
        });
    };
    ...
    UserService.prototype.jwt = function () {
        ...
        if (currentUser && currentUser.auth_token) {
            var headers = new http_1.Headers({ 'Authorization': 'Bearer ' + currentUser.auth_token });
